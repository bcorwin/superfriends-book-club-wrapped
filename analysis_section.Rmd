...
if (!exists("limits")) limits <- c(NA, NA, NA)

if (length(meta) == 1) meta <- rep(meta, 2)
```
...
source("likert.r", local = TRUE)
liker_results <- get_liker()
cat("## Rating distribution", sep = "\n")
```
...
# Note: all time only
max_values <- (
  raters_bookmakers == apply(raters_bookmakers, 1, max, na.rm = TRUE)
)
min_values <- (
  raters_bookmakers == apply(raters_bookmakers, 1, min, na.rm = TRUE)
)
...
for (i in seq_len(nrow(df))) {
  for (j in seq_len(ncol(df))) {
    is_min <- coalesce(min_values[i, j], FALSE)
    is_max <- coalesce(max_values[i, j], FALSE)
    if (is_min) {
      bg_color <- "#67a9cf"
    } else if (is_max) {
      bg_color <- "#ef8a62"
...
```{r, eval = (dataset == "raters") & (params$year > 2020), results = "asis"}
cat("## Renegades", sep = "\n")
source("renegade.r", local = TRUE)
renegade_results <- get_renegade()

cat(paste("Biggest lover:", renegade_results$biggest_lovers), "\n\n")
cat(paste("Biggest hater:", renegade_results$biggest_haters), "\n")
```
```{r, eval = (dataset == "raters") & (params$year > 2020), fig.width=9}
renegade_results$plot
```
```{r, eval = (dataset == "raters") & (params$year > 2020), fig.width=9, results = "asis"}
cat("\n### Renegade voter summary\n")
renegade_results$table
```
