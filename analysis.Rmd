...
rating_data <- read_csv("Data/SF Book Club Ratings - Ratings.csv", show_col_types = FALSE)
meta_data <- read_csv("Data/SF Book Club Ratings - Meta.csv", show_col_types = FALSE) %>%
  select(-Link) %>%
  rename(
...
raters_bookmakers <- get_ratings_summary(c("rater", "bookmaker")) %>%
  ...
  select(period, rater, bookmaker, average) %>%
...
# Meta data (pick a new name)

## Book length
```{r}
plot(density(meta_data$pages))
hist(meta_data$pages)
```

## WIP
```{r}

plot(density(meta_data$year))
plot(density(meta_data$goodreads_rating))
ggplot(meta_data, aes(x=genre)) + geom_bar()
ggplot(meta_data, aes(x=gender)) + 
  geom_bar() +
  labs(
    caption="Other includes Non-binary and multiple others of different genders."
  )

plot_ly(ratings, x=~pages, y=~rating, color = ~rater)  # TODO: Split into short story / novel / essays?
plot_ly(ratings, x=~year, y=~rating, color = ~rater)  # TODO: Split into classic / contemporary?
plot_ly(ratings, x=~goodreads_rating, y=~rating, color = ~rater)
# Box plot of ratings by Genre and Gender

meta_cors <- ratings %>%
  group_by(rater) %>%
  summarise(
    across(c(year, pages, goodreads_rating), ~ cor(.x, rating)),
  )

make_bar_plot(meta_cors, "rater", "pages", color="rater")
make_bar_plot(meta_cors, "rater", "year", color="rater")
make_bar_plot(meta_cors, "rater", "goodreads_rating", color="rater")
...
meta_ratings_diffs %>%
  make_bar_plot("rater", "genre", color="rater")

meta_ratings_diffs %>%
  make_bar_plot("rater", "gender", color="rater")
