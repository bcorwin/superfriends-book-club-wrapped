...
meta_data <- read_csv("~/Downloads/SF Book Club Ratings - Meta.csv", show_col_types = FALSE) %>%
  select(-Link) %>%
  rename(
...
## WIP
```{r}
plot(density(ratings$pages))
plot(density(ratings$year))
plot(density(ratings$goodreads_rating))

plot_ly(ratings, x=~pages, y=~rating, color = ~rater)
plot_ly(ratings, x=~year, y=~rating, color = ~rater)
