...
```{r generate-bookmaker-info}
bookmaker_info <- read_fwf(
  "data/bookmaker_info.txt",
  col_positions = fwf_empty(
    "data/bookmaker_info.txt", 
    col_names = c("bookmaker", "color", "pair")
  ),
  show_col_types = FALSE
)

bookmaker_info$bookmaker <- as.ordered(bookmaker_info$bookmaker)
...
meta_ratings_diffs <- ratings %>%
  ...
  mutate(  # Do not change these orders without updating downstream too
    genre = `Fiction` - `Non-fiction`,
    gender = `Female` - `Male`
  ) %>%
  select(rater, genre, gender)
```
[Source rating data](https://docs.google.com/spreadsheets/d/10o_hAeYeQOCet3tj5Mu2wj8dxeeu9ItoTNb9mbCBjV0/edit?usp=drivesdk)
...
temp <- ratings %>%
  ...
  arrange(date) %>%
  mutate(
    output = glue("{row_number()}. {bookmaker}'s *{book}*: {recap}")
  )
cat(temp$output, sep="\n")
```
